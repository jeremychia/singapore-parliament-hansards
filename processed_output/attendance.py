...
def drop_present_files(csv_filename, attendance_files):
    try:
        ...
        files_processed_df = pd.read_csv(os.path.join(script_dir, csv_filename))
...
for file_name in to_process_files:
    attendance_cur_df = pd.read_csv(os.path.join(attendance_files_dir, file_name))
    ...
    write_csv(os.path.join(script_dir, attendance_filename), attendance_cur_df)
